...
fun TransactionCard(txn: Transaction, secName: String, memberName: String, onDelete: () -> Unit) {
    ...
    Card(shape = RoundedCornerShape(12.dp), modifier = Modifier.fillMaxWidth()) {
        Row(modifier = Modifier.padding(12.dp), verticalAlignment = Alignment.CenterVertically) {
            androidx.compose.material3.Surface(shape = CircleShape, color = if (isBuy) GainColor.copy(0.15f) else LossColor.copy(0.15f), modifier = Modifier.size(40.dp)) {
                Box(contentAlignment = Alignment.Center) {
                    Icon(if (isBuy) Icons.Default.ArrowDownward else Icons.Default.ArrowUpward, null, tint = if (isBuy) GainColor else LossColor, modifier = Modifier.size(20.dp))
                }
            }
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    txn.units?.let { Text("${it.formatAs4Dec()} units @ ₹${txn.price?.formatAs4Dec()}", style = MaterialTheme.typography.labelSmall) }
                }
            }
...
fun AddTransactionScreen(preSelectedSecurityId: Long?, onBack: () -> Unit, vm: TransactionViewModel = hiltViewModel()) {
    ...
    val isUnitBased = selectedSecurity?.securityType in listOf(SecurityType.MUTUAL_FUND, SecurityType.SHARES, SecurityType.BOND, SecurityType.GOI_BOND)
    ...
    Scaffold(
        ...
        bottomBar = {
            Surface(shadowElevation = 8.dp) {
                androidx.compose.material3.Button(
                    onClick = {
                        val t = Transaction(familyMemberId = selectedMember!!.id, securityId = selectedSecurity!!.id, transactionDate = txnDate, transactionType = txnType, units = units.toDoubleOrNull(), price = price.toDoubleOrNull(), amount = calcAmount.toDoubleOrNull(), stampDuty = stampDuty.toDoubleOrNull() ?: 0.0, brokerage = brokerage.toDoubleOrNull() ?: 0.0, stt = stt.toDoubleOrNull() ?: 0.0, folioNumber = folioNumber, notes = notes)
        ...
        LazyColumn(modifier = Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            item {
                Card(shape = RoundedCornerShape(16.dp)) {
                    Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        OutlinedTextField(value = securityQuery, onValueChange = { q -> securityQuery = q; showSearch = true; scope.launch { searchResults = vm.searchSecurities(q) } }, label = { Text("Search Security *") }, trailingIcon = { Icon(Icons.Default.Search, null) }, modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp), singleLine = true)
                    ...
                    Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        if (isUnitBased) {
                            ...
                            if (units.isNotBlank() && price.isNotBlank()) {
                                ...
                                Text("Total: ${FinancialUtils.formatCurrencyFull(totalAmt)}", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            }
                        } else {
            ...
            item { Card(shape = RoundedCornerShape(16.dp)) { Column(modifier = Modifier.padding(16.dp)) { InputField("Notes (Optional)", notes, { notes = it }) } } }
        }
    }
}

private fun Double.formatAs4Dec() = "%.4f".format(this)
