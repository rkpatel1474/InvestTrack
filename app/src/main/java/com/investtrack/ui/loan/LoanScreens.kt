    ...
    Scaffold(
        ...
        bottomBar = {
            Surface(shadowElevation = 8.dp) {
                Button(
                    onClick = {
                        val loan = Loan(id = loanId ?: 0L, familyMemberId = selectedMember!!.id, loanName = loanName, loanType = loanType, lenderName = lenderName, accountNumber = accountNumber, loanAmount = loanAmount.toDoubleOrNull() ?: 0.0, interestRate = interestRate.toDoubleOrNull() ?: 0.0, tenureMonths = tenureMonths.toIntOrNull() ?: 0, disbursementDate = disbursementDate, emiAmount = emiAmount.toDoubleOrNull() ?: computedEMI, processingFee = processingFee.toDoubleOrNull() ?: 0.0, notes = notes)
